...
exports.scenarioRouter = void 0;
const express_1 = require("express");
const scenarioEngine_1 = require("../scenario/scenarioEngine");
const failureEngine_1 = require("../failure/failureEngine");
const scenario_1 = require("../types/scenario");
const router = (0, express_1.Router)();
exports.scenarioRouter = router;
const failureEngine = new failureEngine_1.FailureEngine();
const scenarioEngine = new scenarioEngine_1.ScenarioEngine(failureEngine);
...
router.get('/presets', (req, res) => {
    try {
        ...
        if (tags) {
            ...
            presets = presets.filter(p => tagList.some(tag => p.scenario.tags.includes(tag)));
        }
        res.json({
            success: true,
            data: presets,
...
router.get('/presets/:scenarioId', (req, res) => {
    try {
...
// Execute Scenario
router.post('/:scenarioId/execute', (req, res) => {
    try {
        const { scenarioId } = req.params;
...
// Stop Scenario
router.post('/:scenarioId/stop', (req, res) => {
    try {
        const { scenarioId } = req.params;
...
router.get('/categories/list', (req, res) => {
    try {
        const categories = Object.values(scenario_1.ScenarioCategory);
        const difficulties = Object.values(scenario_1.ScenarioDifficulty);
        const statuses = Object.values(scenario_1.ScenarioStatus);
        res.json({
            success: true,
            data: {
                categories,
...
router.get('/stats', (req, res) => {
    try {
        ...
        const completedExecutions = history.filter(e => e.status === scenario_1.ScenarioStatus.COMPLETED);
        if (completedExecutions.length > 0) {
            const successfulExecutions = completedExecutions.filter(e => e.validationResults && e.validationResults.overallScore > 0.7);
...
router.post('/create', (req, res) => {
    try {
        const { name, description, category, difficulty, tags, config, expectedOutcomes, durationMs } = req.body;
...
// Clone Scenario
router.post('/:scenarioId/clone', (req, res) => {
    try {
        const { scenarioId } = req.params;
        ...
        delete cloned.usage; // Remove usage stats
        res.status(201).json({
            success: true,
            data: cloned,
...
router.get('/export', (req, res) => {
    try {
        ...
        if (includeHistoryBool) {
            data.history = scenarioEngine.getExecutionHistory(1000);
        }
        if (format === 'csv') {
...
function convertScenariosToCSV(presets) {
    if (presets.length === 0)
