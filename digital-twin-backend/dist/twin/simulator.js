...
const coolingController_2 = require("../cooling/coolingController");
...
function simulateTick() {
    ...
    if (coolingController_1.coolingState.active && coolingController_1.coolingState.command) {
        const command = coolingController_1.coolingState.command;
        switch (command.mode) {
            case "AGGRESSIVE":
    ...
    const workloadBoost = (0, heatingController_1.getWorkloadBoost)();
    for (const rack of Object.values(twinState_1.twinState.racks)) {
        // Check if this rack is affected by heating
        const isAffectedByHeating = !heatingController_1.heatingState.active || !heatingController_1.heatingState.command?.affectedRacks ||
            heatingController_1.heatingState.command.affectedRacks.includes(`R${Object.keys(twinState_1.twinState.racks).indexOf(rack) + 1}`);
        ...
        if (isAffectedByHeating && heatingController_1.heatingState.active) {
        ...
        const isAffectedByCooling = !coolingController_1.coolingState.active || !coolingController_1.coolingState.command?.affectedRacks ||
            coolingController_1.coolingState.command.affectedRacks.includes(`R${Object.keys(twinState_1.twinState.racks).indexOf(rack) + 1}`);
        ...
        if (rack.inletTemp > 30)
            rack.status = "HOT"; // Lowered from 32
        else if (rack.inletTemp > 25)
            rack.status = "WARM"; // Lowered from 27
        else
            rack.status = "OK";
    }
    (0, aggregate_1.computeAggregates)(twinState_1.twinState);
    // Update cooling and heating costs based on power consumption
    (0, coolingController_2.updateCoolingCost)(1000); // 1 second tick
    (0, heatingController_1.updateHeatingCost)(1000); // 1 second tick
    const counts = { OK: 0, WARM: 0, HOT: 0 };
    const rackDetails = [];
    for (const [rackId, r] of Object.entries(twinState_1.twinState.racks)) {
        counts[r.status]++;
    ...
    console.log(`Cooling: ${coolingController_1.coolingState.active ? 'ACTIVE' : 'OFF'} | Heating: ${heatingController_1.heatingState.active ? 'ACTIVE' : 'OFF'}`);
    ...
    if (coolingController_1.coolingState.active) {
        console.log(`\n❄️  COOLING STATUS: Mode=${coolingController_1.coolingState.command?.mode || 'UNKNOWN'} | Power=${coolingController_1.coolingState.currentPowerKW.toFixed(2)}kW | Cost=$${coolingController_1.coolingState.totalCost.toFixed(2)}`);
    }
    if (heatingController_1.heatingState.active) {
        console.log(`\n🔥 HEATING STATUS: Mode=${heatingController_1.heatingState.command?.mode || 'UNKNOWN'} | Power=${heatingController_1.heatingState.currentPowerKW.toFixed(2)}kW | Cost=$${heatingController_1.heatingState.totalCost.toFixed(2)}`);
    }
    console.log("=".repeat(80));
    // Keep original simple log for compatibility
    console.log("Status distribution:", counts, "Cooling:", coolingController_1.coolingState.active ? "YES" : "NO", "Heating:", heatingController_1.heatingState.active ? "YES" : "NO");
    return twinState_1.twinState;
}
